<?php
class tagbag_Client_PostTags {
	public function __construct() {
		// Add adv post tags in post ( all / feedonly / blogonly / homeonly / singularonly / singleonly / pageonly /no )
		if ( tagbag_Plugin::get_option_value('tt_embedded') != 'no' || tagbag_Plugin::get_option_value('tt_feed') == 1 ) {
	...
	public static function the_content( $content = '' ) {
		// Get option
		$tt_embedded = tagbag_Plugin::get_option_value('tt_embedded');

		$marker = false;
		if ( is_feed() ) {
			if ( (int) tagbag_Plugin::get_option_value('tt_feed') == 1 ) {
				$marker = true;
			}
	...
	public static function extendedPostTags( $args = '', $copyright = true ) {
		...
		// Default values
		$defaults = array(
			'before'		=> __('Tags: ', 'tagbag'),
			'separator'	=> ', ',
			'after'			=> '<br />',
			'post_id'		=> 0,
			'inc_cats'	=> 0,
			'xformat'		=> __('<a href="%tag_link%" title="%tag_name_attribute%" %tag_rel%>%tag_name%</a>', 'tagbag'),
			'notagtext'	=> __('No tag for this post.', 'tagbag'),
			'number'		=> 0,
			'format'		=> ''
		);
		...
		foreach ( (array) $terms as $term ) {
			...
			$output[] = tagbag_Client::format_internal_tag( $xformat, $term, $rel, null );
		}
		...
		return tagbag_Client::output_content( '', 'string', '', $output, $copyright );
	}
}