...
class tagbag_Admin {
	...
	public function __construct() {
		...
		if ( (int) tagbag_Plugin::get_option_value('use_suggested_tags') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.suggest.php');
			new tagbag_Admin_Suggest();
		}

		if ( (int) tagbag_Plugin::get_option_value('use_click_tags') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.clickterms.php');
			new tagbag_Admin_ClickTags();
		}

		if ( (int) tagbag_Plugin::get_option_value('use_autocompletion') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.autocomplete.php');
			new tagbag_Admin_Autocomplete();
		}

		if ( (int) tagbag_Plugin::get_option_value('active_mass_edit') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.mass.php');
			new tagbag_Admin_Mass();
		}

		if ( (int) tagbag_Plugin::get_option_value('active_manage') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.manage.php');
			new tagbag_Admin_Manage();
		}

		if ( (int) tagbag_Plugin::get_option_value('active_autotags') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.autoterms.php');
			new tagbag_Admin_AutoTags();
		}

		if ( (int) tagbag_Plugin::get_option_value('active_autotags') == 1 || (int) tagbag_Plugin::get_option_value('auto_link_tags') == 1 ) {
			require( TAGB_DIR . '/inc/class.admin.post.php');
			new tagbag_Admin_Post_Settings();
		}
	}
	...
	public static function init() {
		self::$taxo_name = __('Post Tags', 'tagbag');
		self::$post_type_name = __('Posts', 'tagbag');
		...
		if ( !isset($taxo) && is_array($compatible_taxonomies) && !empty($compatible_taxonomies) ) {
			...
			// TODO: Redirect to Help user that see's the URL...
		} elseif( !isset($taxo) ) {
			wp_die( __('This custom post type doesn\'t have taxonomies.', 'tagbag') );
		}
	...
	/**
	 * Build HTML form to allow the user to change the taxonomy for the current page.
	 *
	...
	public static function boxSelectorTaxonomy( $page_value = '' ) {
		echo '<div class="box-selector-taxonomy">' . PHP_EOL;
			echo '<p class="current-taxonomy">'.sprintf(__('You currently use the custom post type "<span>%s</span>" and the taxonomy "<span>%s</span>"', 'tagbag'), self::$post_type_name, self::$taxo_name).'</p>' . PHP_EOL;
			...
			echo '<div class="change-taxo">' . PHP_EOL;
				echo '<form action="" method="get">' . PHP_EOL;
					...
					echo '<input type="submit" class="button" id="submit-change-taxo" value="'.__('Change selection', 'tagbag').'" />' . PHP_EOL;
				echo '</form>' . PHP_EOL;
			echo '</div>' . PHP_EOL;
		echo '</div>' . PHP_EOL;
	}
	...
	/**
	 * Init somes JS and CSS needed for Tag Bag.
	 *
	 * @return void
	 * @author Amaury Balmer
	 */
	public static function admin_enqueue_scripts() {
		...
		// Helper Tag Bag
		wp_register_script('tb-helper-add-tags', TAGB_URL.'/assets/js/helper-add-tags.js', array('jquery'), TAGB_VERSION);
		wp_register_script('tb-helper-options', TAGB_URL.'/assets/js/helper-options.js', array('jquery'), TAGB_VERSION);

		// Register CSS
		wp_register_style('tb-admin', TAGB_URL.'/assets/css/admin.css', array(), TAGB_VERSION, 'all' );
		...
		if (
			...
			( isset($_GET['page']) && in_array($_GET['page'], array('tb_mass_terms', 'tb_auto', 'tb_options', 'tb_manage')) )
		) {
			wp_enqueue_style ('tb-admin');
		}

		// Add jQuery tabs for options page. Use jQuery UI Tabs from WP
		if ( isset($_GET['page']) && $_GET['page'] == 'tb_options' ) {
			wp_enqueue_script('jquery-ui-tabs');
			wp_enqueue_script('tb-helper-options');
		}
	}
	...
	public static function admin_menu() {
		add_options_page( __('Tag Bag: Options', 'tagbag'), __('Tag Bag', 'tagbag'), 'admin_tag_bag', self::menu_slug, array(__CLASS__, 'page_options'));
	...
	public static function page_options() {
		// Get options
		$options = tagbag_Plugin::get_option();

		// Update or reset options
		if ( isset($_POST['updateoptions']) ) {
			check_admin_referer('updateresetoptions-tagbag');
			...
			tagbag_Plugin::set_option( $options );
			add_settings_error( __CLASS__, __CLASS__, __('Options saved', 'tagbag'), 'updated' );
		} elseif ( isset($_POST['reset_options']) ) {
			check_admin_referer('updateresetoptions-tagbag');
			tagbag_Plugin::set_default_option();
			add_settings_error( __CLASS__, __CLASS__, __('Tag Bag options resetted to default options!', 'tagbag'), 'updated' );
		}
		settings_errors( __CLASS__ );
		include( TAGB_DIR . '/views/admin/page-settings.php' );
	}
	...
	/**
	 * Default content for Tag Bag meta box
	 *
	 * @return string
	 * @author Amaury Balmer
	 */
	public static function getDefaultContentBox() {
		if ( (int) wp_count_terms('post_tag', 'ignore_empty=false') == 0 ) { // TODO: Custom taxonomy
			return __('This feature requires at least 1 tag to work. Begin by adding tags!', 'tagbag');
		} else {
			return __('This feature requires JavaScript to be enabled.', 'tagbag');
		}
	}

	/**
	 * A short public static function to display the same copyright on all admin pages
	 *
	 * @return void
	 * @author Amaury Balmer
	 */
	...
	public static function print_options( $option_data ) {
		// Get options
		$option_actual = tagbag_Plugin::get_option();
	...
	public static function getNiceTitleOptions( $id = '' ) {
		switch ( $id ) {
			case 'administration':
				return __('Administration', 'tagbag');
				break;
			case 'auto-links':
				return __('Auto-Link', 'tagbag');
				break;
			case 'features':
				return __('Features', 'tagbag');
				break;
			case 'metakeywords':
				return __('Meta Keyword', 'tagbag');
				break;
			case 'embeddedtags':
				return __('Embedded Tags', 'tagbag');
				break;
			case 'tagspost':
				return __('Current Post Tags', 'tagbag');
				break;
			case 'relatedposts':
				return __('Related Posts', 'tagbag');
				break;
			case 'relatedtags':
				return __('Related Tags', 'tagbag');
				break;
			case 'tagcloud':
				return __('Tag Cloud', 'tagbag');
				break;
		}
		return '';
	}
	...
	/**
	 * This method checks if the DB is up to date, and if an upgrade is needed for options
	...
	public static function upgrade() {
		...
		$current_version = get_option( TAGB_OPTIONS_NAME . '-version' );

		// Upgrade needed?
		if ( $current_version == false || version_compare($current_version, TAGB_VERSION, '<') ) {
			$current_options = get_option( TAGB_OPTIONS_NAME );
			$default_options = (array) include( TAGB_DIR . '/inc/helper.options.default.php' );
			...
			update_option( TAGB_OPTIONS_NAME . '-version', TAGB_VERSION );
			update_option( TAGB_OPTIONS_NAME, $current_options );
		}
	...
	/**
	 * Make a simple SQL query with some args to get terms for ajax display
	 *
	 * @param string $taxonomy
	 * @param string $search
	...
	public static function getTermsForAjax( $taxonomy = 'post_tag', $search = '', $order_by = 'name', $order = 'ASC' ) {
		...
		if ( !empty($search) ) {
			return $wpdb->get_results( $wpdb->prepare("
				SELECT DISTINCT t.name, t.term_id, tt.count, tt.description
