<?php
class tagbag_Client_RelatedPosts {
	/**
	 * Constructor
	 *
	 * @return void
	...
	public function __construct() {
		...
		if ( (tagbag_Plugin::get_option_value('rp_embedded') != 'no') || (tagbag_Plugin::get_option_value('rp_feed') == 1) ) {
	...
	public static function the_content( $content = '' ) {
		// Get option
		$rp_embedded = tagbag_Plugin::get_option_value('rp_embedded');

		$marker = false;
		if ( is_feed() ) {
			if ( (int) tagbag_Plugin::get_option_value('rp_feed') == 1 ) {
				$marker = true;
			}
	...
	public static function get_related_posts( $user_args = '', $copyright = true ) {
		...
		$defaults = array(
			...
			'limit_days'		=> 0,
			'min_shared'		=> 1,
			'title'					=> __('<h4>Related Posts</h4>', 'tagbag'),
			'nopoststext'		=> __('No Related Posts.', 'tagbag'),
			'dateformat'		=> get_option('date_format'),
			'xformat'				=> __('<a href="%post_permalink%" title="%post_title% (%post_date%)">%post_title%</a> (%post_comment%)', 'tagbag')
		);
		...
		if ( $cache = wp_cache_get( 'related_posts'.$taxonomy, 'tagbag' ) ) {
			if ( isset( $cache[$key] ) ) {
				$results = $cache[$key];
			}
		}
		...
		if ( $results === false || $results === null ) {
			...
			if ( $current_terms == false || is_wp_error($current_terms) ) {
				return tagbag_Client::output_content( 'tb-related-posts', $format, $title, $nopoststext, $copyright );
			}
			...
			$cache[$key] = $results;
			wp_cache_set('related_posts'.$taxonomy, $cache, 'tagbag');
		}
		...
		return tagbag_Client::output_content( 'tb-related-posts', $format, $title, $output, $copyright, $separator );
	}
	...
	public static function get_excerpt_post( $excerpt = '', $content = '', $password = '', $excerpt_length = 55 ) {
		if ( !empty($password) ) { // if there's a password
			if ( $_COOKIE['wp-postpass_'.COOKIEHASH] != $password ) { // and it doesn't match the cookie
				return __('There is no excerpt because this is a protected post.', 'tagbag');
			}
		}
	...
	public static function get_tags_from_id( $terms = '', $taxonomy = 'post_tag' ) {
		...
		foreach ( (array) $terms as $term ) {
			...
			$output[] = '<a href="'.$link.'" title="'.esc_attr(sprintf( _n('%d topic', '%d topics', (int) $term->count, 'tagbag'), $term->count )).'" '.$rel.'>'.esc_html($term->name).'</a>';
		}
