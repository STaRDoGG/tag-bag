<?php
class tagbag_Admin_Autocomplete {
	...
	public function __construct() {
		...
		// Tag Bag hook
		add_action( 'tagbag-auto_terms', array(__CLASS__, 'auto_terms_js') );
		add_action( 'tagbag-manage_terms', array(__CLASS__, 'manage_terms_js') );
		add_action( 'tagbag-mass_terms', array(__CLASS__, 'mass_terms_js') );
	...
	public static function admin_enqueue_scripts() {
		...
		// Register JS/CSS
		wp_register_script('tb-helper-autocomplete', TAGB_URL.'/assets/js/helper-autocomplete.js', array('jquery', 'jquery-ui-autocomplete'), TAGB_VERSION);
		...
		if ( (in_array($pagenow, $wp_post_pages) || ( in_array($pagenow, $wp_page_pages) && is_page_have_tags() )) || ( isset($_GET['page']) && in_array( $_GET['page'], array('tb_auto', 'tb_mass_terms', 'tb_manage') ) ) ) {
			wp_enqueue_script('tb-helper-autocomplete');
		}
	}
	...
	public static function ajax_check() {
		if ( isset($_GET['tb_action']) && $_GET['tb_action'] == 'helper_js_collection' )  {
	...
	public static function ajax_local_tags() {
		...
		$terms = tagbag_Admin::getTermsForAjax( $taxonomy, $search );
...
*/
	...
	/**
	 * Content of custom meta box of Tag Bag
	 *
	 * @param object $post
	 * @return void
	 * @author Amaury Balmer
	...
	public static function metabox( $post ) {
		// Get options
		$autocomplete_min = (int) tagbag_Plugin::get_option_value('autocomplete_min');
		?>
		<p>
			<?php if ( tagbag_Plugin::get_option_value('autocomplete_type') == 'textarea' ) : ?>
				<textarea class="widefat" name="adv-tags-input" id="adv-tags-input" rows="3" cols="5"><?php echo tagbag_Admin::getTermsToEdit( 'post_tag', $post->ID ); ?></textarea>
			<?php else : ?>
		...
		<script type="text/javascript">
			<!--
			tb_init_autocomplete( '#adv-tags-input', '<?php echo admin_url("admin-ajax.php?action=tagbag&tb_action=helper_js_collection"); ?>', <?php echo $autocomplete_min; ?> );
			-->
		</script>
		<?php
	}
	...
	public static function auto_terms_js( $taxonomy = '' ) {
		...
		<script type="text/javascript">
			<!--
			tb_init_autocomplete( '#auto_list', "<?php echo admin_url('admin-ajax.php?action=tagbag&tb_action=helper_js_collection&taxonomy='.$taxonomy); ?>", <?php echo $autocomplete_min; ?> );
			-->
		</script>
		<?php
	}
	...
	public static function manage_terms_js( $taxonomy = '' ) {
		...
		<script type="text/javascript">
			<!--
			tb_init_autocomplete( '.autocomplete-input', "<?php echo admin_url('admin-ajax.php?action=tagbag&tb_action=helper_js_collection&taxonomy='.$taxonomy); ?>", <?php echo $autocomplete_min; ?> );
			-->
		</script>
		<?php
	}
