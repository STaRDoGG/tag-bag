<?php
class tagbag_Client_TagCloud {
	public function __construct() {
		// Embedded tag cloud
		if ( (int) tagbag_Plugin::get_option_value('allow_embed_tcloud') == 1 ) {
			add_shortcode( 'tb_tag_cloud', array(__CLASS__, 'shortcode') );
			add_shortcode( 'tb-tag-cloud', array(__CLASS__, 'shortcode') );
		}
	}
	...
	public static function extendedTagCloud($args = '', $copyright = true) {
		...
		if ( empty($terms) ) {
			return tagbag_Client::output_content( 'tb-tag-cloud', $format, $title, $notagstext, $copyright );
		}
		...
		$rel = tagbag_Client::get_rel_attribut();
		...
		if ( $orderby == 'count' ) {
			...
			tagbag_Client::random_array($counts);
		}
		...
		foreach ( (array) $counts as $term_name => $count ) {
			...
			$output[] = tagbag_Client::format_internal_tag( $xformat, $term, $rel, $scale_result, $scale_max, $scale_min, $largest, $smallest, $unit, $maxcolor, $mincolor );
		}
		...
		return tagbag_Client::output_content( 'tb-tag-cloud', $format, $title, $output, $copyright );
	}
	...
	public static function getTags( $args = '', $taxonomy = 'post_tag' ) {
		...
		// Get cache if exist
		if ( $cache = wp_cache_get( 'tb_get_tags', 'tagbag' ) ) {
			if ( isset( $cache[$key] ) ) {
		...
		$cache[$key] = $terms;
		wp_cache_set( 'tb_get_tags', $cache, 'tagbag' );

		$terms = apply_filters('tb_get_tags', $terms, $args);
		return $terms;
	}

	/**
	 * Helper public static function for keep compatibility with old options Tag Bag widgets
	 *
	...
	public static function getTerms( $taxonomies, $args = '' ) {
		...
		$defaults = array('orderby' => 'name', 'order' => 'ASC',
			...
			// Tag Bag added
			'limit_days' => 0, 'category' => 0, 'min_usage' => 0, 'tb_name__like' => '' );
		...
		$latb_changed = wp_cache_get('last_changed', 's-terms');
		if ( !$last_changed ) {
			$last_changed = time();
		...
		if ( strpos($tb_name__like, ' ') !== false ) {

			$tb_terms_formatted = array();
			$tb_terms = preg_split('/[\s,]+/', $tb_name_like);
			foreach ( (array) $tb_terms as $tb_term ) {
				if ( empty($tb_term) )
					continue;
				$tb_terms_formatted[] = "t.name LIKE '%".like_escape($tb_term)."%'";
			}

			$where .= " AND ( " . explode( ' OR ', $tb_terms_formatted ) . " ) ";
			unset( $tb_term, $tb_terms_formatted, $tb_terms );

		} elseif ( !empty($tb_name__like) ) {

			$where .= " AND t.name LIKE '%{$tb_name__like}%'";
