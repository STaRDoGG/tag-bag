...
class tagbag_Admin_Suggest {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_register_script('tb-helper-suggested-tags', TAGB_URL.'/assets/js/helper-suggested-tags.js', array('jquery', 'tb-helper-add-tags'), TAGB_VERSION);
		wp_localize_script('tb-helper-suggested-tags', 'tbHelperSuggestedTagsL10n', array( 'title_bloc' => self::get_suggest_tags_title(), 'content_bloc' => __('Choose a provider to get suggested tags (local, yahoo or tag the net).', 'tagbag') ) );
		...
		if ( in_array($pagenow, $wp_post_pages) || (in_array($pagenow, $wp_page_pages) && is_page_have_tags() ) ) {
			wp_enqueue_script('tb-helper-suggested-tags');
		}
	}
	...
	/**
	 * Register metabox for suggest tags, for post, and optionally page.
	 *
	 * @return void
	 * @author Amaury Balmer
	 */
	public static function admin_menu() {
		add_meta_box('suggestedtags', __('Suggested Tags', 'tagbag'), array(__CLASS__, 'metabox'), 'post', 'advanced', 'core');
		if ( is_page_have_tags() )
			add_meta_box('suggestedtags', __('Suggested Tags', 'tagbag'), array(__CLASS__, 'metabox'), 'page', 'advanced', 'core');
	}
	...
	public static function metabox() {
		...
		<span class="container_clicktags">
			<?php echo tagbag_Admin::getDefaultContentBox(); ?>
			<div class="clear"></div>
		</span>
	    <?php
	}
	...
	public static function ajax_opencalais() {
		...
		// API Key ?
		if ( tagbag_Plugin::get_option_value('opencalais_key') == '' ) {
			echo '<p>'.__('OpenCalais need an API key to work. You can register on service website to obtain a key and set it on Tag Bag options.', 'tagbag').'</p>';
			exit();
		}
		...
		$response = wp_remote_post('http://api.opencalais.com/enlighten/rest/', array('body' => array(
			'licenseID' => tagbag_Plugin::get_option_value('opencalais_key'),
			'content' 	=> $content,
		...
		if ( empty($data) || is_wp_error($response) ) {
			echo '<p>'.__('No results from OpenCalais service.', 'tagbag').'</p>';
			exit();
		}
	...
	private static function _get_params_xml_opencalais() {
		return '
			<c:params xmlns:c="http://s.opencalais.com/1/pred/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
				...
				<c:userDirectives c:allowDistribution="false" c:allowSearch="false" c:externalID="" c:submitter="Tag Bag"></c:userDirectives>
	...
	public static function ajax_alchemy_api() {
		...
		// API Key?
		if ( tagbag_Plugin::get_option_value('alchemy_api') == '' ) {
			echo '<p>'.__('AlchemyAPI need an API key to work. You can register on service website to obtain a key and set it on Tag Bag options.', 'tagbag').'</p>';
			exit();
		}
		...
		$response = wp_remote_post( 'http://access.alchemyapi.com/calls/html/HTMLGetRankedConcepts', array('body' => array(
			'apikey' 	 => tagbag_Plugin::get_option_value('alchemy_api'),
			'maxRetrieve' => 30,
			'html' 		 => $content,
			'outputMode' => 'json',
			'sourceText' => 'cleaned'
		...
		if ( empty($data->concepts) ) {
			echo '<p>'.__('No results from Alchemy API.', 'tagbag').'</p>';
			exit();
		}
	...
	public static function ajax_zemanta() {
		...
		// API Key ?
		if ( tagbag_Plugin::get_option_value('zemanta_key') == '' ) {
			echo '<p>'.__('Zemanta need an API key to work. You can register on service website to obtain a key and set it on Tag Bag options.', 'tagbag').'</p>';
			exit();
		}
		...
		$response = wp_remote_post( 'http://api.zemanta.com/services/rest/0.0/', array('body' => array(
			...
			'api_key'						=> tagbag_Plugin::get_option_value('zemanta_key'),
			'text'							=> $content,
			'format'						=> 'json',
			'return_rdf_links'	=> 0,
			'return_images'			=> 0
		...
		if ( empty($data) ) {
			echo '<p>'.__('No results from Zemanta API.', 'tagbag').'</p>';
			exit();
		}
	...
	public static function ajax_suggest_local() {
		...
		if ( ((int) wp_count_terms('post_tag', 'ignore_empty=false')) == 0) { // No suggestions
		...
		// Get all terms
		$terms = tagbag_Admin::getTermsForAjax( 'post_tag', '' );
		...
		if ( $flag == false ) {
			echo '<p>'.__('No correspondance between your content and terms from the WordPress database.', 'tagbag').'</p>';
		} else {
			echo '<div class="clear"></div>';
		}
		exit();
