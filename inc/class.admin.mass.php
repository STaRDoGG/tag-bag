...
class tagbag_Admin_Mass {
	...
	public static function admin_menu() {
		add_management_page( __('Tag Bag: Mass Edit Terms', 'tagbag'), __('Mass Edit Terms', 'tagbag'), 'tag_bag', 'tb_mass_terms', array(__CLASS__, 'pageMassEditTags'));
	}
	...
	public static function admin_init() {
		...
		if ( isset($_POST['update_mass']) ) {
			...
			if ( ! ( wp_verify_nonce($_POST['secure_mass'], 'tb_mass_terms') ) ) {
			...
			if ( isset($_POST['tags']) ) {
				...
				foreach ( (array) $_POST['tags'] as $object_id => $tag_list ) {
					...
					wp_set_object_terms( $object_id, $tags, tagbag_Admin::$taxonomy );
					$counter++;
				...
				add_settings_error( __CLASS__, __CLASS__, sprintf(__('%1$s %2$s(s) terms updated with success !', 'tagbag'), (int) $counter, strtolower(tagbag_Admin::$post_type_name) ), 'updated' );
				return true;
			}
		}
		return false;
	...
	public static function pageMassEditTags() {
		...
		?>
		<div class="wrap">
			<?php tagbag_Admin::boxSelectorTaxonomy( 'tb_mass_terms' ); ?>

			<form id="posts-filter" action="" method="get">
				<input type="hidden" name="page" value="tb_mass_terms" />
				...
				<h2><?php _e('Mass edit terms', 'tagbag'); ?></h2>

				<ul class="subsubsub">
					<?php
					$status_links = array();
					$num_posts = wp_count_posts(tagbag_Admin::$post_type, 'readable');
					$class = (empty($_GET['post_status']) && empty($_GET['post_type'])) ? ' class="current"' : '';
					$status_links[] = '<li><a href="'.admin_url('tools.php').'?page=tb_mass_terms&amp;cpt='.tagbag_Admin::$post_type.'&amp;taxo='.tagbag_Admin::$taxonomy.'"'.$class.'>'.__('All', 'tagbag').'</a>';
					foreach ( $post_stati as $status => $label ) {
						...
						$status_links[] = '<li><a href="'.admin_url('tools.php').'?page=tb_mass_terms&amp;cpt='.tagbag_Admin::$post_type.'&amp;taxo='.tagbag_Admin::$taxonomy.'&amp;post_status='.$status.'"'.$class.'>' . sprintf(_n($label[2][0], $label[2][1], (int) $num_posts->$status), number_format_i18n( $num_posts->$status )) . '</a>';
					}
				...
				<p class="search-box">
					...
					<input type="submit" value="<?php _e( 'Search', 'tagbag' ); ?>" class="button" />
				</p>
				...
				<div class="tablenav">
					...
					<div style="float: left">
						<?php
						if ( !is_singular() ) {
							$arc_result = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $wpdb->posts WHERE post_type = %s ORDER BY post_date DESC", tagbag_Admin::$post_type ) );
							...
							if ( $month_count && !( 1 == $month_count && 0 == $arc_result[0]->mmonth ) ) { ?>
								<select name='m'>
								<option<?php selected( @$_GET['m'], 0 ); ?> value='0'><?php _e('Show all dates', 'tagbag'); ?></option>
								<?php
						...
						<select name="posts_per_page" id="posts_per_page">
							<option <?php if ( !isset($_GET['posts_per_page']) ) echo 'selected="selected"'; ?> value=""><?php _e('Quantity&hellip;', 'tagbag'); ?></option>
						...
						<input type="submit" id="post-query-submit" value="<?php _e('Filter', 'tagbag'); ?>" class="button-secondary" />
						<?php } ?>
					</div>
			...
			<?php if ( have_posts() ) :
				...
				<form name="post" id="post" method="post">
					<table class="widefat post fixed">
						<thead>
							<tr>
								<th class="manage-column"><?php _e('Post title', 'tagbag'); ?></th>
								<th class="manage-column"><?php printf(__('Terms : %s', 'tagbag'), esc_html(tagbag_Admin::$taxo_name) ); ?></th>
							</tr>
						</thead>
						<tbody>
							<?php
							...
							while (have_posts()) {
								...
								<tr valign="top" class="<?php echo $class; ?>">
									<th scope="row"><a href="<?php echo admin_url('post.php?action=edit&amp;post='.get_the_ID()); ?>" title="<?php _e('Edit', 'tagbag'); ?>"><?php echo ( get_the_title() == '' ) ? the_ID() : the_title(); ?></a></th>
									<td><input id="tags-input<?php the_ID(); ?>" class="autocomplete-input tags_input" type="text" size="100" name="tags[<?php the_ID(); ?>]" value="<?php echo tagbag_Admin::getTermsToEdit( tagbag_Admin::$taxonomy, get_the_ID() ); ?>" /></td>
								</tr>
								<?php
							}
							?>
					...
					<p class="submit">
						<input type="hidden" name="secure_mass" value="<?php echo wp_create_nonce('tb_mass_terms'); ?>" />
						<input class="button-primary" type="submit" name="update_mass" value="<?php _e('Update all &raquo;', 'tagbag'); ?>" />
					</p>
				</form>
		...
		<?php
		do_action( 'tagbag-mass_terms', tagbag_Admin::$taxonomy );
	}
	...
	public static function edit_data_query( $q = false ) {
		...
		// Content type
		$q['post_type'] = tagbag_Admin::$post_type;
		...
		$avail_post_stati = get_available_post_statuses(tagbag_Admin::$post_type);
