<div class="wrap tb_wrap">
	...
	<h2><?php _e('Tag Bag Options', 'tagbag'); ?></h2>
	<h2 class="nav-tab-wrapper">
		<?php

		// Get array options/description
		$option_data = (array) include( TAGB_DIR . '/inc/helper.options.admin.php' );
	...
	<form action="<?php echo self::$admin_url; ?>" method="post">
		...
		<p>
			<?php wp_nonce_field( 'updateresetoptions-tagbag' ); ?>
			<input class="button-primary" type="submit" name="updateoptions" value="<?php _e('Update Options &raquo;', 'tagbag'); ?>" />
			<input class="button" type="submit" name="reset_options" onclick="return confirm('<?php _e('Do you really want to restore the default options?', 'tagbag'); ?>');" value="<?php _e('Reset Options', 'tagbag'); ?>" />
		</p>
	</form>
